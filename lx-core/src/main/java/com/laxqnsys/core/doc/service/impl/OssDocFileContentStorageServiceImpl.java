...
import com.laxqnsys.core.properties.OssFileUploadProperties;
import com.laxqnsys.core.util.oss.OssUtils;
...
public class OssDocFileContentStorageServiceImpl extends AbstractFileSystemStorageService {
    ...
    public OssDocFileContentStorageServiceImpl(LxDocWebProperties lxDocWebProperties) {
        ...
        OssFileUploadProperties oss = docStorage.getOss();
    ...
    public boolean copy(List<DocFileFolder> fileFolders) {
        fileFolders.forEach(fileFolder -> {
            ...
            try {
                CopyObjectRequest request = new CopyObjectRequest(this.bucket, srcFilePath, this.bucket, targetFilePath);
                this.ossClient.copyObject(request);
            } catch (Exception e) {
    ...
    public boolean delete(DocFileFolder docFileFolder) {
        ...
        try {
            ...
            throw new BusinessException(ErrorCodeEnum.ERROR.getCode(), String.format("删除文件【%s】失败", objectName),
                e);
        }
        return true;
    }
    ...
    private OSSObject getFile(String objectName) {
        try {
            OSSObject ossObject = this.ossClient.getObject(this.bucket, objectName);
            return ossObject;
        } catch (Exception e) {
